...
public class LatinSquare {
	 ...
	 * @version 1.1
	 * @since Lab #1
	 */ https://udel.instructure.com/courses/1439803/files/folder/3.0%20Labs/Lab%20%232/doc?preview=75050557
	...
	protected boolean hasDuplicates() {
		for(int i = 0;i<LatinSquare.length;i++) {
			if(hasDuplicates(getColumn(i)) || hasDuplicates(getRow(i)))
				return true;
		}
		return false;
	}
	...
	private int[] RemoveZeros(int[] arr) {
		...
		int[] arrCopy = new int[nonZeroCount];
		int acIndex = 0;
		for(int i =0;i<arr.length;i++) {
			if(arr[i] != 0) {
				arrCopy[acIndex] = arr[i];
				acIndex++;
			}
		}
	}
